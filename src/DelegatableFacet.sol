...
contract DelegatableFacet is IDelegatable, DelegatableCore {
    ...
    function getDelegationTypedDataHash(Delegation memory delegation) public view returns (bytes32) {
        bytes32 digest =
            keccak256(abi.encodePacked("\x19\x01", getEIP712DomainHash(), GET_DELEGATION_PACKETHASH(delegation)));
        return digest;
    }
    ...
    function getInvocationsTypedDataHash(Invocations memory invocations) public view returns (bytes32) {
        bytes32 digest =
            keccak256(abi.encodePacked("\x19\x01", getEIP712DomainHash(), GET_INVOCATIONS_PACKETHASH(invocations)));
        return digest;
    }
    ...
    /// @inheritdoc IDelegatable
    function invoke(SignedInvocation[] calldata signedInvocations) external override returns (bool success) {
